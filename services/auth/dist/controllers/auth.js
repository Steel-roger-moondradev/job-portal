...
export const forgetPassword = TryCatch(async (req, res, next) => {
    ...
    const resetToken = jwt.sign({
        emailId: user.email,
        type: "reset",
    }, process.env.RESET_PASS_TKN, { expiresIn: "15m", });
    ...
    publishToProducer(process.env.KAFKA_TOPIC, message).catch((error) => { console.log("failed to send message"); });
    res.json("Mail has been send if email exists");
});
export const resetPassword = TryCatch(async (req, res, next) => {
    ...
    let decoded;
    try {
        decoded = jwt.verify(token, process.env.RESET_PASS_TKN);
    }
    catch (error) {
